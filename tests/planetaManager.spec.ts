...
describe('PlanetaManager - Suite de Pruebas', () => {
  let manager: PlanetaManager;
  let dimManager: DimensionManager;
  let planetaBase: Planetas;
  let dimBase: Dimension;
  let jsonBase: IPlanetaJSON;
  let mockDMInstance: { leerBaseDatos: unknown; guardarBaseDatos: unknown };

  beforeEach(() => {
    vi.clearAllMocks();

    dimBase = new Dimension('D-1', 'Cronenberg World', 'activa', 1, 'Mundo mutante');
    planetaBase = new Planetas('P1', 'Tierra', 'Planeta', dimBase, 7000, 'Hogar');
